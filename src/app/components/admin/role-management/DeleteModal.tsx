...
export function DeleteModal({ role, onConfirm, onClose }: { role: Role; onConfirm: () => void; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
        ...
        <h3 className="text-white text-center mb-1" style={{ fontSize: "1rem", fontWeight: 600 }}>Xóa Vai Trò</h3>
        <p className="text-[#8899aa] text-center mb-5" style={{ fontSize: "0.85rem" }}>
          Bạn có chắc chắn muốn xóa <span className="text-white font-mono">{role.code}</span>? Hành động này không thể hoàn tác.
        </p>
